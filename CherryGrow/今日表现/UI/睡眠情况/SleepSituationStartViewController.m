...
- (void) sleepSituationListLoaded:(NSArray<SleepSituation*>*) situations{
    [situations enumerateObjectsUsingBlock:^(SleepSituation * _Nonnull sleepSituation, NSUInteger i, BOOL * _Nonnull stop) {
        [self replaceSituation:sleepSituation];
    }];
}

- (void) replaceSituation:(SleepSituation*) situation{
    [self.sleepSituations enumerateObjectsUsingBlock:^(SleepSituation * _Nonnull sleepSituation, NSUInteger idx, BOOL * _Nonnull stop) {
        if (situation.code == sleepSituation.code) {
            [self.sleepSituations replaceObjectAtIndex:idx withObject:situation];
            *stop = YES;
        }
    }];
}
...
- (void) submitSleepSituation:(SleepSituationParam*) situation{
    ...
    [SituationRequestManager createAddSleepSituationRequest:situation.code status:situation.status success:^(id result) {
        ...
        __strong typeof(self) strongSelf = weakSelf;
        SleepSituation* sleepSituation = (SleepSituation*) result;
        [strongSelf replaceSituation:sleepSituation];
    }failed:^(NSInteger errorCode, NSString *message) {
